...
def affichage_victoire(niveau, temps_ecoule, nb_fusions, son_fin):
    ...
    while act:
        ...
        if etape == 0:
            texte_victoire = police_fin_niv.render("Victoire !", True, VICTOIRE_TEXT)
        ...
        if etape == 0 and temps_ecoule_ms >= 500:
            ...
            txt_temps = police_options.render(f"Temps écoulé : {minutes} minutes et {secondes} secondes", True, VICTOIRE_TEXT)
            ecr.blit(txt_temps, (lrg // 2 - txt_temps.get_width() // 2, 150))
            etape = 2
            temps_debut = time.get_ticks()
        
        elif etape == 2 and temps_ecoule_ms >= 1000:
            txt_fusions = police_options.render(f"Nombre de fusions : {nb_fusions}", True, VICTOIRE_TEXT)
            ecr.blit(txt_fusions, (lrg // 2 - txt_fusions.get_width() // 2, 200))
            etape = 3
            temps_debut = time.get_ticks()
        
        elif etape == 3 and temps_ecoule_ms >= 1000:
            bouton(ecr, BOUTON_REINIT_FOND, btn_quit, "Quitter", son_survol, son_clicmenu, 10, surbrillance=BOUTON_REINIT_SURVOL)
            etape = 4
