...
# Fonction d'assombrissement progressif avec effet de dessèchement
def assombrir_image(surface, burn_level, max_factor_r=0.6, max_factor_g=0.4, max_factor_b=0.2):
    ...
    # Détection des zones dominées par le vert
    green_mask = (g > r) & (g > b)
    
    # Application des facteurs uniquement sur les zones vertes
    ...
    # Conversion de l'image PIL en pygame.Surface
...
def page_jeu(niveau):
    ...
    while act:
        try:
            ...
            # Tant que le feu (id 22) est en collision avec un objet (autre que feu ou eau),
            # on augmente lentement le burn_level et on met à jour l'image avec l'effet d'assombrissement.
            # Si un objet eau (id 8) est appliqué sur un objet brûlé, on rétablit l'image originale et on retire l'eau.
